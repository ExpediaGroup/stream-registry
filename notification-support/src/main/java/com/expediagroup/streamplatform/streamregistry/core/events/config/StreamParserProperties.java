...
import static com.expediagroup.streamplatform.streamregistry.core.events.config.NotificationEventConfig.CUSTOM_STREAM_TYPE_PREFIX;
...
@Data
@ConfigurationProperties(prefix = CUSTOM_STREAM_TYPE_PREFIX)
public class StreamParserProperties {
  ...
  public Function<Stream, ?> buildStreamToKeyRecord() {
  ...
  public Function<Stream, ?> buildStreamToValueRecord() {
    return Optional.ofNullable(customEnabled)
        .filter(Boolean::booleanValue)
  ...
  private <R extends SpecificRecord> Function<Stream, R> loadValueParser() {
    Objects.requireNonNull(valueParserClass, getWarningMessageOnNotDefinedProp("enabled stream type parser", CUSTOM_STREAM_VALUE_PARSER_CLASS_PROPERTY));
    Objects.requireNonNull(valueParserMethod, getWarningMessageOnNotDefinedProp("enabled stream type parser", CUSTOM_STREAM_VALUE_PARSER_METHOD_PROPERTY));

    try {
      return NotificationEventUtils.loadToAvroStaticMethod(valueParserClass, valueParserMethod, Stream.class);
    } catch (Exception e) {
      throw new IllegalStateException(e);
    }
  }
