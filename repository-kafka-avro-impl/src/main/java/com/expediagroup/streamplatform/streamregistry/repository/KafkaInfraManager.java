...
public class KafkaInfraManager implements InfraManager {
  ...
  private ZkUtils initZkUtils(Properties config) {
    String zkConnect = config.getProperty(ZOOKEEPER_QUORUM);
  ...
  private Materialized createMaterialized(String schemaRegistryUrl) {
    ...
    final SpecificAvroSerde<com.expediagroup.streamplatform.streamregistry.ClusterKey> keySpecificAvroSerde = new SpecificAvroSerde<>();
    keySpecificAvroSerde.configure(serdeConfig, true);

    final SpecificAvroSerde<com.expediagroup.streamplatform.streamregistry.ClusterValue> valueSpecificAvroSerde = new SpecificAvroSerde<>();
    valueSpecificAvroSerde.configure(serdeConfig, false);

    return Materialized.<com.expediagroup.streamplatform.streamregistry.ClusterKey, com.expediagroup.streamplatform.streamregistry.ClusterValue, KeyValueStore<Bytes, byte[]>>as(infraStateStoreName)
